...
try:
    ...
    if mysql_connection.is_connected():
        logging.info("STFC MMU enrollment query")
        # Get the current date and time
        current_time_start = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        print( f" pushing Enrollment start . { current_time_start }" )
        ...
        print("STFC MMU db enrollment query") 
        ...
        AND ( i_ben_mapping.CreatedDate between '{oneDayBeforeTodayStartDate}' and '{todayEndDate}'
